...
def boundary_equation_builder(side, boundary, span):
    ...
    if _type == BoundaryType.FIXED:
        ...
        return fdm.create_bc('equation', opts['operator'], free_value=opts.get('value', 0.))
    else:
...
class Truss1d(Truss1d):
    def __init__(self, length, nodes_number):
        fdm.builder.Truss1d.__init__(self, length, nodes_number)

        self._boundary_builder = boundary_equation_builder

        self._alpha = 0.8
        self._length_scale = length * 0.1
        self._resolution = None

        self._length_scale_corrector = None
        self._fractional_operator_pattern = {
            'central': "CCC",
            'backward': "CCC",
            'forward': "CCC",
        }
    ...
    def set_fractional_operator_pattern(self, **kwargs):
        self._fractional_operator_pattern.update(kwargs)
        return self

    def set_length_scale_corrector(self, _type, *args, **kwargs):
        self._length_scale_corrector = LENGTH_SCALES_CORRECTORS[_type](
            self._length, self._length_scale, *args, **kwargs)
        return self

    def create(self):
        if self._length_scale_corrector is None:
            self.set_length_scale_corrector('null')
        return super().create()
    ...
    def _create_operators_set(self, stiffness_multiplier):
        fractional_deformation_operator_central = fdm.DynamicElement(
            self._create_fractional_operator_builder(
                self._fractional_operator_pattern['central']
            )
        )
        fractional_deformation_operator_backward = fdm.DynamicElement(
            self._create_fractional_operator_builder(
                self._fractional_operator_pattern['backward']
            )
        )
        fractional_deformation_operator_forward = fdm.DynamicElement(
            self._create_fractional_operator_builder(
                self._fractional_operator_pattern['forward']
            )
        )

        fractional_ep_central = fdm.Operator(
            fdm.Stencil.central(span=self._span),
            stiffness_multiplier * fdm.Number(self._young_modulus) * fractional_deformation_operator_central
        )
        fractional_ep_backward = fdm.Operator(
            fdm.Stencil.backward(span=self._span),
            stiffness_multiplier * fdm.Number(self._young_modulus) * fractional_deformation_operator_backward
        )
        fractional_ep_forward = fdm.Operator(
            fdm.Stencil.forward(span=self._span),
            stiffness_multiplier * fdm.Number(self._young_modulus) * fractional_deformation_operator_forward
        )
    ...
    def _create_fractional_operator_builder(self, pattern):
        dynamic_lf = self._length_scale_corrector

        def dynamic_resolution(point):
            return int(dynamic_lf(point) / self._span) if self._resolution is None else self._resolution

        def create_stencil(point):
            return fractional_mechanics.create_riesz_caputo_operator_by_pattern(
                fractulus.CaputoSettings(self._alpha, dynamic_lf(point), dynamic_resolution(point)),
                pattern,
                self._span
            ).to_stencil(Point(0))
